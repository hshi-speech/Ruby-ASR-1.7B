...
# Ruby-ASR-ver: verbatim-style Japanese ASR with inline furigana + a CTC mora-reading head

<!-- EDIT before uploading: replace HF_NAMESPACE with your HF user/org name. -->

Ruby-ASR-ver is a fine-tune of [Qwen/Qwen3-ASR-1.7B](https://huggingface.co/Qwen/Qwen3-ASR-1.7B)
for **verbatim-style** Japanese transcription. A sibling checkpoint,
[`HF_NAMESPACE/Ruby-ASR-sub`](https://huggingface.co/HF_NAMESPACE/Ruby-ASR-sub),
targets subtitle-style transcription; both share the same architecture, mora
...
1. **Ruby (furigana) transcription** — the standard Qwen3-ASR seq2seq decoder
   emits the transcript with inline bracket furigana, annotated per word:
...
vllm serve HF_NAMESPACE/Ruby-ASR-ver --served-model-name ruby-asr --max-model-len 8192
```
...
model = Qwen3ASRModel.from_pretrained("HF_NAMESPACE/Ruby-ASR-ver",
...
rec = MoraCTCRecognizer.from_pretrained("HF_NAMESPACE/Ruby-ASR-ver")  # reads ctc/ subfolder
...
- Verbatim-style output: the transcript aims to follow the spoken content
  closely — for condensed subtitle-style output use
  [`HF_NAMESPACE/Ruby-ASR-sub`](https://huggingface.co/HF_NAMESPACE/Ruby-ASR-sub).
